...
Cypress.Commands.add('loggingIn', (username, password) => {
 ...
 /*  cy.pause(); */
  cy.intercept('GET', 'https://assessment-dev-api.taxd.co.uk/api/csrf-token').as('loginSuccessi')
      cy.wait('@loginSuccessi').its('response.statusCode').should('eq', 200) 
  cy.elementIsVisible('h1.TopBar_header-text__lEb_i')
  cy.elementIsVisible('.Sidebar_sidebar__PBjfv')
  cy.elementIsVisible('.ButtonGrid_button-grid__RvCbo')
  cy.elementIsVisible('.blur-background')
